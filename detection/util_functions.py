...
def links2subgraphs(
    ...
    if h == "auto":
        ...
        print(
            "\033[91mValidation AUC of AA is {}, CN is {}\033[0m".format(
...
def subgraph_extraction_labeling(
    ind, A, h=1, max_nodes_per_hop=None, node_information=None
):
...
def node_label(subgraph):
    ...
    labels = (
        1
        + np.minimum(dist_to_0, dist_to_1).astype(int)
...
def generate_node2vec_embeddings(
    A, emd_size=128, negative_injection=False, train_neg=None
):
    ...
    model = Word2Vec(
        walks, size=emd_size, window=10, min_count=0, sg=1, workers=8, iter=1
    )
    wv = model.wv
